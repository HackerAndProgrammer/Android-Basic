...
import com.example.imcupgradedactivity.imcupgradedcalculator.ResultImcUpgradedActivity
...
class ImcUpgradedCalculatorActivity : AppCompatActivity() {
    ...
    private var currentWeight: Int =
        70                               //We set a default weight (The user will change it)
    ...
    private fun initListeners() {
        ...
        btnCalculate.setOnClickListener {                               //Is the button for the IMC calculation pressed?
            val result =
                calculateIMC() //We set the result variable with the function for the IMC calculating
    ...
    private fun navigateToResult(result: Double) {    //This is the function that will send the user to the next screen
        val intent = Intent(
            this,
            ResultImcUpgradedActivity::class.java
        )    //We set an Intent so that the next screen will catch it
        intent.putExtra(
            IMC_KEY,
            result
        )                                          //We add the IMC_KEY, and the IMC already calculated to the Intent
